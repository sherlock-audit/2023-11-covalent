...
describe('Test BRP contract and staking contract interaction via bridge agent', function () {
    beforeEach(async function () {
        [contractsAndAccounts, parameters] = await setupWithDefaultParameters({ withProofChain: 'BlockResult' });
        [
            owner,
            stakingContract,
            cqtContract,
    ...
    it('calls addBRPOperator on PC, triggers enableValidator on SC (sufficient stake), which triggers enableValidator on PC', async function () {
        ...
        await proofChain.connect(owner).addBRPOperator(operator.address, 1);
    ...
    it('calls addBRPOperator on PC, triggers enableValidator on SC (but fails when insufficient stake), and doesn\'t trigger enableValidator on PC', async function () {
        validator = validators[1];
        operator = operators[1];
        // test with 1 index rather than 0 (making sure that test doesn't pass due to some default value of 0 somewhere)
        await stakingContract.connect(stakingManager).addValidator(validators[0].address, commissionRate);
    ...
    it('call removeBRPOperator on PC, activeOpCount=0, triggers disableValidator on SC, and the same on PC', async function () {
        validator = validators[0];
        ...
        await proofChain.connect(owner).addBRPOperator(operator1.address, 0);

        await waitForEvent(proofChain, "ValidatorEnabled", [0]);

        await proofChain.connect(owner).removeBRPOperator(operator1.address);
        await waitForEvent(stakingContract, "ValidatorDisabled", [0]);
        await waitForEvent(proofChain, "ValidatorDisabled", [0]);
    });

    it('call removeBRPOperator on PC, activeOpCount>0, triggers nothing on SC', async function () {
        validator = validators[0];
        operator1 = operators[0];
        ...
        await proofChain.connect(owner).addBRPOperator(operator1.address, 0);
        await proofChain.connect(owner).addBRPOperator(operator2.address, 0);
    ...
    it('SC.stake should trigger enableValidator on SC if proofchain.validator is enabled + active operators are there', async function () {
        ...
        await waitForEvent(proofChain, "OperatorAdded", [operator1.address, 0, BLOCK_RESULT_PRODUCER_ROLE]);
        await ensureNoEvent(stakingContract, "ValidatorEnabled");
    ...
    it('SC.stake shouldn\'t trigger SC.enableValidator if proofchain.validator is enabled + but no active operators', async function () {
        ...
        // no active operators
        //proofChain.connect(owner).addBRPOperator(operator1.address, 0);
        // await proofChain.connect(validator).enableBRPOperator(operator1.address);
    ...
    async function ensureNoEvent(contract, eventName) {
        ...
        // it makes the tests using this very slow (and maybe flaky), but it's better than nothing
