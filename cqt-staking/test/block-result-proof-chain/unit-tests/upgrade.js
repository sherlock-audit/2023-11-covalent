...
describe('Initialize contract', function() {
  it('works before and after upgrading', async function() {
    ...
    const pc = await ethers.getContractFactory( 'BlockResultProofChain', owner);
    ...
    await sss.addBRPOperator("0x004cEBf70ef85C93e1F742BA3A7Aa00076138855", 3)
    await sss.getBRPRoleData()
    await expect(await sss.owner()).to.equal(owner.address);

    const SSS2 = await ethers.getContractFactory('BlockResultProofChain', owner);
    sss2 = await upgrades.upgradeProxy(sss.address, SSS2,);
    console.log('BlockResultProofChain upgraded to:', sss2.address);
